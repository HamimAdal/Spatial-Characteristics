...
We do not propose to solve all these problems, at least not for now.  However, we do want to make progress towards allowing users to exploit the capabilities of smart spaces with a lot less effort and cognitive burden.  Our approach starts with a simple observation: the interactions with the smart spaces fall into two large categories:

-- First, it is often the case that users want and need to interface with specific devices and the features they provide.  The user may want to preheat the oven, check the baby monitor, or watch a movie on TV.  
...
<b>2. The Spatial Characteristics model redefines and broadens the domain of spatial context offering solutions to effectively achieve user requests?</b>
